...
def construct_embeddings(n_vocab, rand_dim, pretrained_dim=0, usage=ModelUsage.INIT, ignore_label=None):
    if pretrained_dim <= 0 or usage == ModelUsage.NONE:
        rand_model = L.EmbedID(n_vocab, rand_dim, ignore_label=ignore_label)
        pretrained_model = None

    elif usage == ModelUsage.CONCAT or usage == ModelUsage.ADD:
        rand_model = L.EmbedID(n_vocab, rand_dim, ignore_label=ignore_label)
        pretrained_model = L.EmbedID(n_vocab, pretrained_dim, ignore_label=ignore_label)

    elif usage == ModelUsage.INIT:
        rand_model = L.EmbedID(n_vocab, pretrained_dim, ignore_label=ignore_label)
        pretrained_model = None
...
def construct_RNN(unit_type, bidirection, n_layers, n_input, n_units, dropout):
    ...
    print('# RNN unit: {}, dropout={}'.format(rnn, rnn.__dict__['dropout']), file=sys.stderr)
    for i, c in enumerate(rnn._children):
        print('#   {}-th param'.format(i), file=sys.stderr)
        print('#      0 - W={}, b={}'.format(c.w0.shape, c.b0.shape), file=sys.stderr) 
        print('#      1 - W={}, b={}'.format(c.w1.shape, c.b1.shape), file=sys.stderr) 

        if unit_type == 'gru' or unit_type == 'lstm':
            print('#      2 - W={}, b={}'.format(c.w2.shape, c.b2.shape), file=sys.stderr) 
            print('#      3 - W={}, b={}'.format(c.w3.shape, c.b3.shape), file=sys.stderr) 
            print('#      4 - W={}, b={}'.format(c.w4.shape, c.b4.shape), file=sys.stderr) 
            print('#      5 - W={}, b={}'.format(c.w5.shape, c.b5.shape), file=sys.stderr) 

        if unit_type == 'lstm':
            print('#      6 - W={}, b={}'.format(c.w6.shape, c.b6.shape), file=sys.stderr) 
            print('#      7 - W={}, b={}'.format(c.w7.shape, c.b7.shape), file=sys.stderr) 
...
def grow_embedding_layers(
        ...
        pretrained_model_usage=ModelUsage.NONE, train=False, fasttext=False):
    ...
    if external_model and pretrained_model_usage != ModelUsage.NONE:
        if pretrained_model_usage == ModelUsage.INIT:
            grow_embedding_layer_with_pretrained_model(
                n_vocab_org, n_vocab_grown, rand_embed,
                external_model, id2unigram_grown, train=train, fasttext=fasttext)

        else:
            grow_embedding_layers_with_pretrained_model(
                n_vocab_org, n_vocab_grown, rand_embed, pretrained_embed, 
                external_model, id2unigram_grown, train=train)

    else:
        grow_embedding_layer_without_pretrained_model(
            n_vocab_org, n_vocab_grown, rand_embed, train=train)
        

def grow_embedding_layer_without_pretrained_model(
        n_vocab_org, n_vocab_grown, rand_embed, train=False):
    ...
    print('Grow embedding matrix: {} -> {}'.format(n_vocab_org, rand_embed.W.shape[0]), file=sys.stderr)


# rand model       -> grow using external model
def grow_embedding_layer_with_pretrained_model(
        n_vocab_org, n_vocab_grown, rand_embed, 
        external_model, id2unigram_grown, train=False, fasttext=False):
    diff = n_vocab_grown - n_vocab_org
    d_rand = rand_embed.W.shape[1]
    ...
    print('Grow embedding matrix: {} -> {}'.format(n_vocab_org, rand_embed.W.shape[0]), file=sys.stderr)
    if count >= 1:
        print('Add {} pretrained embedding vectors'.format(count), file=sys.stderr)
...
def grow_embedding_layers_with_pretrained_model(
        n_vocab_org, n_vocab_grown, rand_embed, pretrained_embed, 
        external_model, id2unigram_grown, train=False):
    diff = n_vocab_grown - n_vocab_org
    d_rand = rand_embed.W.shape[1]
    ...
    print('Grow pretrained embedding matrix: {} -> {}'.format(
        n_vocab_org, pretrained_embed.W.shape[0]), file=sys.stderr)
    if count >= 1:
        print('Add {} pretrained embedding vectors'.format(count), file=sys.stderr)
...
def grow_crf_layer(n_labels_org, n_labels_grown, crf, file=sys.stderr):
    ...
    print('Grow CRF layer: {} -> {}'.format(c_org.shape, crf.cost.shape, file=sys.stderr))
...
def grow_MLP(n_labels_org, n_labels_grown, out_layer, file=sys.stderr):
    ...
    print('Grow MLP output layer: {}, {} -> {}, {}'.format(
        w_org_shape, b_org_shape, w_shape, b_shape), file=sys.stderr)
